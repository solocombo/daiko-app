...
function calcItem(item, jpyRate, proxyRate) {
  ...
  return {
    ...
    totalPrice,
    profit,
    isCustomPrice: item.custom_unit_price != null,
  };
}
...
export default function BatchDetail({
  ...
  const [cardChargeForm, setCardChargeForm] = useState({
    twd_amount: "",
    jpy_amount: "",
    category: "商品",
    note: "",
  ...
  function saveColOrder(order) {
    setColOrder(order);
    localStorage.setItem(COLS_KEY, JSON.stringify(order));
  }
    ...
    await supabase.from("card_charges").insert([
      {
        ...
        jpy_amount: cardChargeForm.jpy_amount
          ? parseFloat(cardChargeForm.jpy_amount)
          : null,
  ...
  function orderTotalDue(order) {
    ...
    const productTotal = activeItems.reduce(
      (s, i) => s + calcItem(i, jpyRate, proxyRate).totalPrice,
      0,
    );
  ...
  function orderPaid(orderId) {
    return (allPayments[orderId] || []).reduce(
  ...
  async function saveOrder() {
    ...
    if (form.items.some((i) => !i.name || !i.jpy_price))
      return alert("請填寫所有商品名稱與日幣價格");
    setSaving(true);
    try {
  ...
  async function applyShipping() {
    ...
    await Promise.all(
      orders.map((o) =>
        supabase
          .from("orders")
          .update({ shipping_twd: Math.round(shippingPerOrder[o.id] || 0) })
          .eq("id", o.id),
      ),
    );
  ...
  async function addPaymentRecord() {
    ...
    if (totalPaid >= due) {
      await supabase
        .from("orders")
        .update({ product_paid: true, shipping_paid: true })
    ...
    if (totalPaid > due) {
      ...
      const { data: existing } = await supabase
        ...
        .select("*")
        .eq("customer", paymentOrder.customer)
        .single();
      if (existing) {
        await supabase
          .from("customer_credits")
          .update({
            balance: existing.balance + excess,
        ...
        await supabase
          .from("customer_credits")
          .insert([{ customer: paymentOrder.customer, balance: excess }]);
      }
    }
    setPaymentAmount("");
    setPaymentNote("");
  ...
  async function deletePaymentRecord(id) {
    ...
    if (totalPaid < orderTotalDue(paymentOrder)) {
      await supabase
        .from("orders")
        .update({ product_paid: false, shipping_paid: false })
        .eq("id", paymentOrder.id);
    }
    fetchAllPayments();
    onRefresh();
  ...
  async function saveDomesticShipping() {
    ...
    if (editingDs) {
      await supabase
        .from("domestic_shipping_jp")
        .update({
          amount_jpy: parseFloat(dsForm.amount_jpy),
          note: dsForm.note,
        })
        .eq("id", editingDs.id);
    } else {
  ...
  const domesticShippingPerOrder = useMemo(() => {
    ...
    const totalJpy = domesticShippings.reduce(
      (s, d) => s + Number(d.amount_jpy),
      0,
    );
    const totalTwd = totalJpy * jpyRate;
    return orders.reduce((acc, o) => {
      const cnt = (o.order_items || [])
        .filter((i) => !i.not_obtained)
        .reduce((s, i) => s + (Number(i.quantity) || 1), 0);
  ...
  useEffect(() => {
    ...
    orders.forEach((o) =>
      (o.order_items || []).forEach((i) => {
        if (i.custom_unit_price != null)
          map[i.id] = String(i.custom_unit_price);
      }),
    );
    setCustomPrices(map);
  }, [orders]);
  ...
  async function handleCustomPriceBlur(itemId, value) {
    ...
    await supabase
      .from("order_items")
      .update({ custom_unit_price: parsed })
      .eq("id", itemId);
  ...
  function openEditCharge(c) {
    setEditingCharge(c);
    setEditChargeForm({
      twd_amount: String(c.twd_amount),
      jpy_amount: c.jpy_amount ? String(c.jpy_amount) : "",
      category: c.category,
      note: c.note || "",
    });
  }
  ...
  async function saveEditCharge() {
    await supabase
      ...
      .update({
        ...
        jpy_amount: editChargeForm.jpy_amount
          ? parseFloat(editChargeForm.jpy_amount)
          : null,
  ...
  async function syncInventory() {
    ...
    setSyncing(false);
    alert(
      `同步完成！新增了 ${added} 筆庫存${added === 0 ? "（可能都已存在）" : ""}`,
    );
  }
    ...
    orders.forEach((o) => {
      ...
      activeItems.forEach((item, idx) => {
        ...
        rows.push([
          ...
          idx === 0 ? due : "",
          idx === 0 ? Math.round(paid) : "",
          idx === 0 ? Math.max(0, due - paid) : "",
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          >
            ...
            <button className="btn-edit-batch" onClick={openBatchEdit}>
              ✏️ 編輯批次
            </button>
          </div>
          <p className="page-sub">
            {batch.date}　 商品匯率 <strong>{jpyRate}</strong>　 運費匯率{" "}
            <strong>{shippingRate}</strong>　 代購匯率{" "}
            <strong>{proxyRate}</strong>　
            {batchShopName !== "—" && (
              <>
                購買網站 <strong>{batchShopName}</strong>　
              </>
            )}
            {domesticShippings.length > 0 && (
              <>
                ...
                <strong>
                  ¥
                  {Math.round(
                    domesticShippings.reduce(
                      (s, d) => s + Number(d.amount_jpy),
                      0,
                    ),
                  ).toLocaleString()}
                </strong>
                （{domesticShippings.length}筆）　
              </>
            )}
          </p>
        </div>
        <div className="header-actions">
          ...
          <button
            className="btn-card-charge"
            onClick={() => setShowDomesticShipping(true)}
          >
            🚚 境內運費
          </button>
          <button
            className="btn-card-charge"
            onClick={() => setShowCardCharges(true)}
          >
            💳 刷卡記錄
          </button>
          <button className="btn-export" onClick={exportBatchCSV}>
            ⬇ 匯出 CSV
          </button>
          <button
      ...
      <div className="profit-banner">
        <div className="profit-item">
          ...
          <span className="profit-value twd">
            NT${Math.round(profit.totalPrice).toLocaleString()}
          </span>
        </div>
        <div className="profit-item">
          <span className="profit-label">實際總成本</span>
          <span className="profit-value neg">
            -NT${Math.round(profit.totalRealCost).toLocaleString()}
          </span>
        </div>
        <div className="profit-item">
          <span className="profit-label">吸收運費</span>
          <span className="profit-value neg">
            -NT${profit.absorbed.toLocaleString()}
          </span>
        </div>
        <div className="profit-item highlight">
          <span className="profit-label">淨利</span>
          <span className="profit-value net">
            NT${Math.round(profit.net).toLocaleString()}
          </span>
        </div>
        {cardCharges.length > 0 && (
          ...
          >
            ...
            <span className="profit-value" style={{ color: "var(--accent)" }}>
              NT$
              {Math.round(
                cardCharges.reduce((s, c) => s + Number(c.twd_amount), 0),
              ).toLocaleString()}
            </span>
          </div>
        )}
      ...
      {orders.length === 0 ? (
        ...
        (() => {
          ...
          const sortedOrders = [...orders].sort((a, b) => {
            if (!sortCol) return 0;
            const activeA = (a.order_items || []).filter(
              (i) => !i.not_obtained,
            );
            const activeB = (b.order_items || []).filter(
              (i) => !i.not_obtained,
            );
            let va, vb;
            switch (sortCol) {
              case "customer":
                ...
                break;
              case "qty":
                va = activeA.reduce((s, i) => s + (Number(i.quantity) || 1), 0);
                vb = activeB.reduce((s, i) => s + (Number(i.quantity) || 1), 0);
                break;
              case "totalPrice":
                va = activeA.reduce(
                  (s, i) => s + calcItem(i, jpyRate, proxyRate).totalPrice,
              ...
              case "profit":
                va = activeA.reduce(
                  (s, i) => s + calcItem(i, jpyRate, proxyRate).profit,
                  0,
                );
                vb = activeB.reduce(
                  (s, i) => s + calcItem(i, jpyRate, proxyRate).profit,
              ...
              case "paid":
                ...
                break;
              case "remaining":
                va = orderTotalDue(a) - orderPaid(a.id);
                vb = orderTotalDue(b) - orderPaid(b.id);
                break;
              case "weight":
                va = activeA.reduce((s, i) => s + Number(i.weight_g || 0), 0);
                vb = activeB.reduce((s, i) => s + Number(i.weight_g || 0), 0);
                break;
              default:
                return 0;
            }
          ...
          function renderCell(col, order) {
            ...
            const totalOrderPrice = activeItems.reduce(
              (s, i) => s + calcItem(i, jpyRate, proxyRate).totalPrice,
              0,
            );
            ...
            switch (col) {
              case "ops":
                return (
                  <td key={col} className="center">
                    <button
                      className="btn-record-payment-sm"
                ...
                return (
                  <td key={col}>
                    <div className="customer-name">
                      {order.customer}
                      {skipProfitCustomers.has(order.customer) && (
                        <span className="skip-profit-badge">不計利潤</span>
                      )}
                    </div>
                    {order.note && (
                  ...
                  <td key={col}>
                    <div className="item-list-detail">
                      {(order.order_items || []).map((item, idx) => (
                        <div
                          key={idx}
                          className={`item-detail-row ${item.not_obtained ? "item-not-obtained" : ""}`}
                        >
                          ...
                          <button
                            ...
                            onClick={() =>
                              toggleNotObtained(item.id, item.not_obtained)
                            }
                          >
                            {item.not_obtained ? "↩" : "✕"}
                          </button>
                  ...
                  <td key={col} className="center">
                    <div className="item-list-detail">
                      {activeItems.map((item, idx) => (
                        <div key={idx} className="multi-val">
                          {getShopName(item.shop_id)}
                        </div>
                      ))}
                    </div>
              ...
              case "jpyPrice":
                return (
                  <td key={col} className="number">
                    {activeItems.map((i, idx) => (
                      <div key={idx} className="multi-val">
                        ¥{Number(i.jpy_price).toLocaleString()}
                      </div>
                    ))}
                  </td>
                );
              case "cost":
                return (
                  <td key={col} className="number">
                    ...
                    {Math.round(
                      activeItems.reduce(
                        (s, i) => s + calcItem(i, jpyRate, proxyRate).costTwd,
                        0,
                      ),
                    ).toLocaleString()}
                  </td>
              ...
              case "fee":
                return (
                  <td key={col} className="number neg">
                    NT$
                    {Math.round(
                      activeItems.reduce(
                        (s, i) => s + calcItem(i, jpyRate, proxyRate).ccFee,
                        0,
                      ),
                    ).toLocaleString()}
                  </td>
              ...
              case "rebate":
                return (
                  <td key={col} className="number green">
                    NT$
                    {Math.round(
                      activeItems.reduce(
                        (s, i) => s + calcItem(i, jpyRate, proxyRate).ccRebate,
                        0,
                      ),
                    ).toLocaleString()}
                  </td>
              ...
              case "realCost":
                return (
                  <td key={col} className="number">
                    ...
                    {Math.round(
                      activeItems.reduce(
                        (s, i) => s + calcItem(i, jpyRate, proxyRate).realCost,
                        0,
                      ),
                    ).toLocaleString()}
                  </td>
                  ...
                  <td key={col} className="number">
                    {activeItems.map((i, idx) => {
                      ...
                      const displayItem =
                        localVal !== undefined
                          ? {
                              ...i,
                              custom_unit_price:
                                localVal === "" ? null : parseFloat(localVal),
                            }
                          : i;
                      ...
                      return (
                        <div
                          key={idx}
                          className="multi-val unit-price-cell"
                        ...
                        >
                          <input
                            ...
                            title="手動覆蓋定價（留空用計算值）"
                            placeholder={String(
                              ceilTo10(Number(i.jpy_price || 0) * proxyRate),
                            )}
                            value={
                              localVal !== undefined
                                ? localVal
                                : i.custom_unit_price != null
                                  ? String(i.custom_unit_price)
                                  : ""
                            }
                            onChange={(e) =>
                              handleCustomPriceChange(i.id, e.target.value)
                            }
                            onBlur={(e) =>
                              handleCustomPriceBlur(i.id, e.target.value)
                            }
                            onClick={(e) => e.stopPropagation()}
                          />
                          {c.isCustomPrice && (
                            <span className="custom-price-badge">手動</span>
                          )}
                        </div>
                      );
                    })}
              ...
              case "totalPrice":
                return (
                  <td key={col} className="number twd">
                    NT${Math.round(totalOrderPrice).toLocaleString()}
                  </td>
                );
              case "profit":
                return (
              ...
              case "forwarder":
                return (
                  <td key={col} className="center">
                    <span className="forwarder-tag">{fwName}</span>
                  </td>
                );
              case "shipping":
                return (
                  <td key={col} className="number">
                    {order.shipping_twd > 0
                      ? `NT$${Math.round(order.shipping_twd).toLocaleString()}`
                      : "—"}
                  </td>
                );
              case "payMethod":
                return (
                  <td key={col} className="center">
                    <span
                      className={`method-tag ${order.payment_method === "取付" ? "method-cod" : ""}`}
                    >
              ...
              case "due":
                return (
                  <td key={col} className="number">
                    NT${due.toLocaleString()}
                  </td>
                );
              case "paid":
                return (
                  <td key={col} className="number net">
              ...
              case "remaining":
                return (
                  <td
                    key={col}
                    className={`number ${remaining > 0 ? "neg" : "net"}`}
                  >
                    {remaining > 0
                      ? `NT$${Math.round(remaining).toLocaleString()}`
                      : "✓"}
                  </td>
                );
              default:
              ...
              <table className="orders-table">
                  ...
                  <tr>
                    {colOrder.map((col) => (
                      ...
                      >
                        ...
                        {sortableSet.has(col) && (
                          <span
                            className={`sort-icon ${sortCol === col ? "active" : ""}`}
                          >
                            {sortCol === col
                              ? sortDir === "asc"
                                ? "▲"
                ...
                <tbody>
                  {sortedOrders.map((order) => {
                    const isFullyPaid =
                      orderPaid(order.id) >= orderTotalDue(order) &&
                      orderTotalDue(order) > 0;
                    const allNotObtained =
                      (order.order_items || []).length > 0 &&
                      (order.order_items || []).every((i) => i.not_obtained);
                    return (
                      <tr
                        key={order.id}
                        className={
                          isFullyPaid || allNotObtained ? "row-done" : ""
                        }
                      >
            ...
            <div className="form-grid">
              ...
              <label className="form-label">
                ...
                <div className="rate-input-row">
                  <input
                    ...
                    type="number"
                    step="0.0001"
                    value={batchForm.jpy_rate}
                    onChange={(e) =>
                      setBatchForm((f) => ({ ...f, jpy_rate: e.target.value }))
                    }
                  />
                  <button
                    className="btn-fetch-rate"
                    onClick={() => fetchRate("jpy_rate")}
                    disabled={fetchingRate === "jpy_rate"}
                  >
                    {fetchingRate === "jpy_rate" ? "⏳" : "🔄 自動抓取"}
                  </button>
                </div>
              </label>
              <label className="form-label">
                ...
                <div className="rate-input-row">
                  ...
                  <button
                    className="btn-fetch-rate"
                    onClick={() => fetchRate("shipping_rate")}
                    disabled={fetchingRate === "shipping_rate"}
                  >
                    {fetchingRate === "shipping_rate" ? "⏳" : "🔄 自動抓取"}
                  </button>
                </div>
              </label>
              ...
              <label className="form-label">
                ...
                <input
                  className="form-input"
                  type="number"
                  value={batchForm.absorbed_shipping_twd}
                  onChange={(e) =>
                    setBatchForm((f) => ({
                      ...f,
                      absorbed_shipping_twd: e.target.value,
                    }))
                  }
                />
              </label>
              <label className="form-label" style={{ gridColumn: "1/-1" }}>
                ...
                <div className="forwarder-select-row">
                  <select
                    className="form-input"
                    value={batchForm.shop_id}
                    onChange={(e) =>
                      setBatchForm((f) => ({ ...f, shop_id: e.target.value }))
                    }
                  >
              ...
              <label className="form-label" style={{ gridColumn: "1/-1" }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBatchForm((f) => ({ ...f, note: e.target.value }))
                  }
                />
              </label>
            </div>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="shipping-calc">
              <div className="calc-summary">
                ...
                <div>
                  總重量（排除未搶到）：<strong>{totalWeightG}g</strong>
                </div>
              </div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn-primary" onClick={applyShipping}>
                套用到所有訂單
              </button>
            </div>
          </div>
        ...
        >
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="payment-due-summary">
              {(() => {
                const due = orderTotalDue(paymentOrder);
                const paid = paymentRecords.reduce(
                  (s, p) => s + Number(p.amount),
                  0,
                );
                const rem = due - paid;
                return (
                  <>
                    <div className="due-row">
                      <span>應收總計</span>
                      <strong>NT${due.toLocaleString()}</strong>
                    </div>
                    <div className="due-row">
                      <span>已收</span>
                    ...
                    <div
                      className={`due-row ${rem > 0 ? "neg-text" : "net-text"}`}
                    >
                      <span>尚欠</span>
                      <strong>
                        {rem > 0
                          ? `NT$${Math.round(rem).toLocaleString()}`
                          : "✓ 已結清"}
                      </strong>
                    </div>
                  </>
                  ...
                  editingPayment?.id === p.id ? (
                    <div key={p.id} className="payment-record-row editing-row">
                      ...
                      <input
                        ...
                        style={{ width: "100px" }}
                      />
                      <button className="btn-edit-sm" onClick={saveEditPayment}>
                        ✓
                      </button>
                      <button
                        className="btn-secondary"
                    ...
                    <div key={p.id} className="payment-record-row">
                      ...
                      <span className="pr-amount net-text">
                        NT${Number(p.amount).toLocaleString()}
                      </span>
                      <button
                        className="btn-edit-sm"
          ...
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-grid">
              <label className="form-label">
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  <input
                    className="form-input"
                    placeholder="例：小花、IG@xxx"
                    value={form.customer}
                    onChange={(e) => setFormField("customer", e.target.value)}
                    style={{ flex: 1 }}
                  />
                  <button
                    type="button"
                    className="btn-preset-customer"
                    onClick={() => setFormField("customer", "現貨")}
                  >
                    現貨
                  </button>
                </div>
              ...
              <label className="form-label">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormField("payment_method", e.target.value)
                  }
                >
              ...
              <label className="form-label">
                ...
                <div className="forwarder-select-row">
                  <select
                    ...
                    onChange={(e) =>
                      setFormField("forwarder_id", e.target.value)
                    }
                  >
                    <option value="">未指定</option>
                    {forwarders.map((fw) => (
            ...
            <div className="items-section">
              ...
              {form.items.map((item, idx) => {
                const suggestions = Object.keys(itemSuggestions)
                  .filter(
                    (n) =>
                      n
                        .toLowerCase()
                ...
                return (
                  <div
                    key={idx}
                    className={`item-row ${item.not_obtained ? "item-not-obtained" : ""}`}
                  >
                    <div className="item-name-wrap">
                      ...
                      <datalist id={`item-suggestions-${idx}`}>
                        {Object.keys(itemSuggestions).map((n) => (
                          <option key={n} value={n} />
                        ))}
                      </datalist>
                      {suggestions.length > 0 &&
                        item.name &&
                        !itemSuggestions[item.name] && (
                            ...
                            {suggestions.map((n) => (
                                ...
                                onClick={() => {
                                  ...
                                  setForm((f) => {
                                    ...
                                    items[idx] = {
                                      ...items[idx],
                                      name: n,
                                      jpy_price: s.jpy_price,
                                      weight_g: s.weight_g,
                                      shop_id: s.shop_id || items[idx].shop_id,
                                    };
                                    return { ...f, items };
                                  });
                                }}
                              >
                                ...
                                <span className="suggestion-price">
                                  ¥{Number(s.jpy_price).toLocaleString()}
                                </span>
                              </div>
                            ))}
                          </div>
                    ...
                    <div className="item-price-wrap">
                      ...
                      <input
                        ...
                        type="number"
                        placeholder="日幣單價"
                        value={item.jpy_price}
                        onChange={(e) =>
                          setItem(idx, "jpy_price", e.target.value)
                        }
                      />
                    </div>
                    <div className="item-weight-wrap">
                      <input
                        className="form-input item-weight"
                        type="number"
                        placeholder="重量"
                        value={item.weight_g}
                        onChange={(e) =>
                          setItem(idx, "weight_g", e.target.value)
                        }
                      />
                    ...
                    <select
                      ...
                      value={item.shop_id || ""}
                      onChange={(e) => setItem(idx, "shop_id", e.target.value)}
                    >
                      <option value="">未指定</option>
                      {shops.map((s) => (
                        <option key={s.id} value={s.id}>
                    ...
                    <label className="not-obtained-check">
                      <input
                        type="checkbox"
                        checked={item.not_obtained || false}
                        onChange={(e) =>
                          setItem(idx, "not_obtained", e.target.checked)
                        }
                      />
                      未搶到
                    </label>
      ...
      {showDomesticShipping && (
        <div
          className="modal-overlay"
          onClick={() => setShowDomesticShipping(false)}
        >
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div>
            ...
            {domesticShippings.length > 0 && (
              <div className="card-charge-summary">
                ...
                <div className="cc-sum-item">
                  <span>總件數</span>
                  <span>{totalItemCount} 件</span>
                </div>
                <div className="cc-sum-total">
                  <span>每件分攤</span>
                ...
                editingDs?.id === d.id ? (
                  <div key={d.id} className="payment-record-row editing-row">
                    <input
                      ...
                      type="number"
                      placeholder="¥"
                      value={dsForm.amount_jpy}
                      onChange={(e) =>
                        setDsForm((f) => ({ ...f, amount_jpy: e.target.value }))
                      }
                      style={{ width: "120px" }}
                    />
                    <input
                      ...
                      value={dsForm.note}
                      onChange={(e) =>
                        setDsForm((f) => ({ ...f, note: e.target.value }))
                      }
                      style={{ flex: 1 }}
                    />
                    <button
                      className="btn-edit-sm"
                      onClick={saveDomesticShipping}
                    >
                      ✓
                    </button>
                    <button
                  ...
                  <div key={d.id} className="payment-record-row">
                    ...
                    <span className="pr-amount twd-text">
                      ¥{Number(d.amount_jpy).toLocaleString()}
                    </span>
                    <span className="pr-date">
                      → NT$
                      {Math.round(
                        Number(d.amount_jpy) * jpyRate,
                      ).toLocaleString()}
                    </span>
                    <button
                      className="btn-edit-sm"
                      onClick={() => {
                        setEditingDs(d);
                        setDsForm({
                          amount_jpy: String(d.amount_jpy),
                          note: d.note || "",
                        });
                      }}
                    >
                    ...
                    <button
                      className="btn-danger-sm"
                      onClick={() => deleteDomesticShipping(d.id)}
                    >
                      刪除
                    </button>
                  </div>
            ...
            {domesticShippings.length > 0 && orders.length > 0 && (
              <div className="cc-add-section" style={{ marginBottom: "8px" }}>
                ...
                {orders.map((o) => {
                  ...
                  if (!cnt) return null;
                  return (
                    <div key={o.id} className="cc-sum-item">
                      <span>
                        {o.customer}（{cnt}件）
                      </span>
                      <span className="twd-text">
                        NT${share.toLocaleString()}
                      </span>
                    </div>
                  );
                })}
            ...
            <div className="modal-footer">
              ...
              <button className="btn-primary" onClick={saveDomesticShipping}>
                新增
              </button>
            </div>
          </div>
      ...
      {showCardCharges && (
        <div
          className="modal-overlay"
          onClick={() => setShowCardCharges(false)}
        >
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div>
            ...
            {cardCharges.length > 0 && (
              <div className="card-charge-summary">
                {["商品", "國際運費", "境內運費", "其他"].map((cat) => {
                  const total = cardCharges
                    .filter((c) => c.category === cat)
                    .reduce((s, c) => s + Number(c.twd_amount), 0);
                  if (!total) return null;
                  return (
                    <div key={cat} className="cc-sum-item">
                      <span>{cat}</span>
                      <span className="twd-text">
                  ...
                  editingCharge?.id === c.id ? (
                    <div key={c.id} className="payment-record-row editing-row">
                      ...
                      <input
                        ...
                        type="number"
                        placeholder="¥"
                        value={editChargeForm.jpy_amount}
                        onChange={(e) =>
                          setEditChargeForm((f) => ({
                            ...f,
                            jpy_amount: e.target.value,
                          }))
                        }
                        style={{ width: "90px" }}
                      />
                      <input
                        ...
                        type="number"
                        placeholder="NT$"
                        value={editChargeForm.twd_amount}
                        onChange={(e) =>
                          setEditChargeForm((f) => ({
                            ...f,
                            twd_amount: e.target.value,
                          }))
                        }
                        style={{ width: "100px" }}
                      />
                      <button className="btn-edit-sm" onClick={saveEditCharge}>
                        ✓
                      </button>
                      <button
                        className="btn-secondary"
                    ...
                    <div key={c.id} className="payment-record-row">
                      ...
                      {c.jpy_amount && (
                        <span className="pr-date">
                          ¥{Number(c.jpy_amount).toLocaleString()}
                        </span>
                      )}
                      <span className="pr-amount twd-text">
                        NT${Number(c.twd_amount).toLocaleString()}
                      </span>
                      <button
                        className="btn-edit-sm"
              ...
              <div className="form-grid">
                ...
                <label className="form-label">
                  ...
                  <input
                    ...
                    type="number"
                    placeholder="NT$"
                    value={cardChargeForm.twd_amount}
                    onChange={(e) =>
                      setCardChargeForm((f) => ({
                        ...f,
                        twd_amount: e.target.value,
                ...
                <label className="form-label">
                  ...
                  <input
                    ...
                    type="number"
                    placeholder="¥"
                    value={cardChargeForm.jpy_amount}
                    onChange={(e) =>
                      setCardChargeForm((f) => ({
                        ...f,
                        jpy_amount: e.target.value,
                ...
                <label className="form-label">
                  ...
                  <input
                    className="form-input"
                    placeholder="例：Animate 訂單"
                    value={cardChargeForm.note}
                    onChange={(e) =>
                      setCardChargeForm((f) => ({ ...f, note: e.target.value }))
                    }
                  />
                </label>
              </div>
          ...
          >
            ...
            <div className="summary-cards">
              {orders.map((order) => {
                ...
                const inputAmt =
                  summaryPayments[order.id] ??
                  (owed > 0 ? String(Math.round(owed)) : "0");
                  ...
                  <div key={order.id} className="summary-card">
                        ...
                        return (
                          <div key={idx} className="summary-item-row">
                            ...
                            <span className="summary-item-calc">
                              NT${c.unitPrice.toLocaleString()} × {c.qty} ={" "}
                              <strong>
                                NT${c.totalPrice.toLocaleString()}
                              </strong>
                            </span>
                          </div>
                        );
                    ...
                    <div className="summary-totals">
                      <div className="summary-total-row">
                        <span>商品總額</span>
                        <span className="twd-text">
                          NT${Math.round(productTotal).toLocaleString()}
                        </span>
                      </div>
                      {alreadyPaid > 0 && (
                        <div className="summary-total-row">
                          ...
                          <span className="net-text">
                            -NT${Math.round(alreadyPaid).toLocaleString()}
                          </span>
                        </div>
                      )}
                      <div className="summary-total-row summary-owed">
                        <span>尚欠</span>
                        <span className={owed > 0 ? "neg-text" : "net-text"}>
                          {owed > 0
                            ? `NT$${Math.round(owed).toLocaleString()}`
                            : "✓ 已結清"}
                        </span>
                      </div>
                    </div>
                    {owed > 0 && (
                      <div className="summary-collect-row">
                        ...
                        />
                        <button
                          className="btn-primary summary-collect-btn"
                          onClick={() =>
                            recordSummaryPayment(order.id, inputAmt)
                          }
                        >
                          記錄
                        </button>
